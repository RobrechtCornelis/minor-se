...
export type Query<T, C> = {   // Bij de functie zijn T en C in het begin hetzelfde <Student,Student>. bij select wordt C een Subset >>>> <Student, Subset<Student,key>
    ...
    template: T[]
    current: C[]
    // currentKeys: currentKeys draagt keys van T die worden geselecteerd via de select of include methode
    currentKeys: (keyof T)[]
    ...
    include: <k extends keyof C & ArrayPropertyNames<T>>(attribute: k, f: (q2: Query<NestedObject<T, k>, NestedObject<C, k>>) => Query<NestedObject<T, k>, any>) => Query<T, Subset<C, k>>
}

type NestedObject<C, k extends keyof C> = { [K in k]: C[K] extends (infer U)[] ? U : never }[k]
...
export const Query = function <T>(array: T[]): Query<T, T> { // new Query<Student, Student>().select("Name", "Surname") -> Query<Student, Pick<{Name:string, Surname:string}>>
    return {   // De Query functie wordt alleen gebruikt voor het initieren van een query, in het begin en niet als return functie bij de methoden. 
        ...
        current: [],
        currentKeys: [],
        // Als er geen select is gedaan (currentKeys.length == 0), dan is de new_array gelijk aan de template (originele data)
        // Als er wel een select is, dan kijkt het naar de opgeslagen keys in currentKeys en maakt een new_array op basis van de geselecteerde keys.
        toArray: function (this: Query<T, T>): T[] {
            let new_array: T[] = []
            this.currentKeys.length == 0 ? new_array = this.template :
                this.template.forEach(element => {
                    let new_element = {} as T;
                    this.currentKeys.forEach(key =>  new_element[key] = element[key])
        ...
        select: function <k extends keyof T>(this: Query<T, T>, ...keys: k[]): Query<T, Subset<T, k>> {
            keys.forEach(key => this.currentKeys.push(key))
        ...
        include: function <k extends ArrayPropertyNames<T>>(this: Query<T, T>, key: k, f: (q2: Query<NestedObject<T, k>, NestedObject<T, k>>) => Query<NestedObject<T, k>, any>): Query<T, T> {
            //     // attribute = 'grades'
            //     // [Grade, Grade, Grade]
            this.currentKeys.push(key)  // "grades" toevoegen aan lijst, zodat niet meer gebruikt kan worden
            this.template.forEach(element => {  // Grade[]
                var temp: NestedObject<T, k>[] = element[key] as NestedObject<T, k>[]
                var queried_array : NestedObject<T, k>[] = f(Query<NestedObject<T, k>>(temp)).toArray()
                element[key] = queried_array as T[k]
            })
            return this
        },
    }
